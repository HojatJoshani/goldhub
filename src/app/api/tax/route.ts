...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!(await isDbAvailable())) {
      return NextResponse.json(getDemoTaxReport(period, startDate, endDate));
    }
    ...
    try {
      ...
      console.error("Tax DB error, using demo:", dbError);
      return NextResponse.json(getDemoTaxReport(period, startDate, endDate));
    }
  } catch (error) {
...
/**
 * Demo tax report data for when database is not available (Vercel)
 */
function getDemoTaxReport(period: string, startDate: Date, endDate: Date) {
  ...
  const VAT_RATE = 0.09;
  const GOLD_MAKING_TAX_RATE = 0.03;
