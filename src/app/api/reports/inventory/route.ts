...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!(await isDbAvailable())) {
      return NextResponse.json(getDemoInventory());
    }
    ...
    try {
      ...
      console.error("Reports inventory DB error, using demo:", dbError);
      return NextResponse.json(getDemoInventory());
    }
  } catch (error) {
...
/**
 * Demo inventory report data for when database is not available (Vercel)
 */
function getDemoInventory() {
  const products = DEMO_PRODUCTS.map((p) => ({
    ...p,
    costPrice: p.costPrice || p.purchasePrice,
    createdAt: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000),
  }));

  const purchaseValue = products.reduce((s, p) => s + p.purchasePrice * p.stock, 0);
  const retailValue = products.reduce((s, p) => s + p.salePrice * p.stock, 0);
  const totalUnits = products.reduce((s, p) => s + p.stock, 0);
  const totalSkus = products.length;

  const karatMap = new Map<string, { weight: number; units: number; products: number }>();
  for (const p of products) {
    const entry = karatMap.get(p.karat) || { weight: 0, units: 0, products: 0 };
    entry.weight += p.weight * p.stock;
    entry.units += p.stock;
    entry.products += 1;
    karatMap.set(p.karat, entry);
  ...
  const categoryName = (id: string | null) =>
    (id && DEMO_CATEGORIES.find((c) => c.id === id)?.name) || "بدون دسته";

  const categoryMap = new Map<string, { products: number; units: number; value: number }>();
  for (const p of products) {
    const key = p.categoryId || "uncat";
    const entry = categoryMap.get(key) || { products: 0, units: 0, value: 0 };
    entry.products += 1;
    entry.units += p.stock;
    entry.value += p.salePrice * p.stock;
    categoryMap.set(key, entry);
  ...
  for (const p of products) {
    if (p.stock === 0) continue;
    const days = Math.floor((now.getTime() - p.createdAt.getTime()) / (24 * 60 * 60 * 1000));
    const bucket = ageBucket(days);
    const entry = agingMap.get(bucket) || { units: 0, value: 0 };
    entry.units += p.stock;
    entry.value += p.purchasePrice * p.stock;
