...
import { DEMO_SALES, DEMO_USERS, DEMO_BRANCHES } from "@/lib/demo-data";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!(await isDbAvailable())) {
      return NextResponse.json(getDemoStaffReport(from, to));
    }
    ...
    try {
      ...
      console.error("Reports staff DB error, using demo:", dbError);
      return NextResponse.json(getDemoStaffReport(from, to));
    }
  } catch (error) {
...
/**
 * Demo staff report data for when database is not available (Vercel)
 */
function getDemoStaffReport(from: Date, to: Date) {
  const users = DEMO_USERS.filter((u) => u.status === "active");
  const branchName = (id: string | null) =>
    (id && DEMO_BRANCHES.find((b) => b.id === id)?.name) || "—";

  // Aggregate sales per cashier
  const salesMap = new Map<string, { total: number; subtotal: number; makingTotal: number; discount: number; count: number }>();
  const salesInRange = DEMO_SALES.filter(
    (s) => s.createdAt >= from && s.createdAt <= to && s.status === "completed"
  );
  for (const s of salesInRange) {
    const entry = salesMap.get(s.cashierId) || { total: 0, subtotal: 0, makingTotal: 0, discount: 0, count: 0 };
    entry.total += s.total;
    entry.subtotal += s.subtotal;
    entry.makingTotal += (s as any).makingTotal || 0;
    entry.discount += s.discount;
    entry.count += 1;
    salesMap.set(s.cashierId, entry);
  }

  const staff = users.map((u) => {
    const s = salesMap.get(u.id) || { total: 0, subtotal: 0, makingTotal: 0, discount: 0, count: 0 };
    return {
      id: u.id,
      name: u.name,
      role: u.role,
  ...
  const totalSalesAll = staff.reduce((s, x) => s + x.salesCount, 0);
  const activeCashiers = staff.filter((s) => s.salesCount > 0).length;
  const avgOrderAll = totalSalesAll > 0 ? totalRevenueAll / totalSalesAll : 0;
  const topPerformer = staff.find((s) => s.salesCount > 0) || null;
