...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!(await isDbAvailable())) {
      return NextResponse.json(getDemoCustomersReport());
    }
    ...
    try {
      ...
      console.error("Reports customers DB error, using demo:", dbError);
      return NextResponse.json(getDemoCustomersReport());
    }
  } catch (error) {
...
/**
 * Demo customers report data for when database is not available (Vercel)
 */
function getDemoCustomersReport() {
  const now = new Date();
  const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
  thirtyDaysAgo.setHours(0, 0, 0, 0);
  ...
  for (let i = 0; i < 14; i++) {
    ...
    const next = new Date(d);
    next.setDate(next.getDate() + 1);
    const count = customers.filter((c) => c.createdAt >= d && c.createdAt < next).length;
    const label = new Intl.DateTimeFormat("fa-IR", {
      day: "numeric",
      month: "short",
    }).format(d);
